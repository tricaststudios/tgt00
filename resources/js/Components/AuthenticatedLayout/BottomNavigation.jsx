import { ChartBarSquareIcon, CreditCardIcon, HomeIcon, ServerIcon, UserIcon } from '@heroicons/react/24/solid';
...
export default function BottomNavigation({ auth }) {
    const navigation = [
        ...
        { name: 'Market', href: route('markets.index'), icon: ChartBarSquareIcon, current: route().current('markets.index') },
        {
            ...
            icon: CreditCardIcon,
            current: route().current('user.wallet.*') || route().current('user.deposits.*') || route().current('user.withdrawals.*'),
        },
